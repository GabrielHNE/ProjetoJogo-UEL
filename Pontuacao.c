...
void pontuacao(int Ox,int Oy,int *score);
void delay(unsigned int milliseconds);
void gotoxy(int x, int y);
void mapaDraw(int mapa[30][30]);
void printMapa(int map[30][30],int i,int j);
					
	void printPac(int Nx, int Ny, int Ox, int Oy,int *score);
...
int main(int argc, char** argv){
	...
	while(!gameOver){
		...
		printPac(pacman.posX, pacman.posY, pacman.oldX, pacman.oldY,&score);
		if(kbhit()){
			keepMove = tecla;
			tecla = getch();
