...
int main(int argc, char** argv){
	...
	while(!gameOver){
        ...
        printPac(ghost.posX, ghost.posY, ghost.oldX, ghost.oldY);
...
void movGhost(Ghost* ghost, Pacman* pacman){
	int dist_menor;
  	...
  	if((mapa[(*ghost).posY][(*ghost).posX  - 1] == 1 || mapa[(*ghost).posY][((*ghost).posX - 60) - 1] == 0) && (calculadist(((*ghost).posX -1),(*ghost).posY,(*pacman).posX,(*pacman).posY) < dist_menor))
  	{
      GHx_menor = ((*ghost).posX  - 1);
      GHy_menor = (*ghost).posY;
  
  	if((mapa[(*ghost).posY][((*ghost).posX-60) - +1] == 1 || mapa[(*ghost).posY][((*ghost).posX - 60) - 1] == 0) && (calculadist(((*ghost).posX +1),(*ghost).posY,(*pacman).posX,(*pacman).posY) < dist_menor))
  	{
      GHx_menor = ((*ghost).posX -60) + 1;
      GHy_menor = (*ghost).posY;
  	}
  	if((mapa[(*ghost).posY-1][((*ghost).posX - 60)] == 1 || mapa[(*ghost).posY][((*ghost).posX-60) - 1] == 0) && (calculadist((*ghost).posX ,((*ghost).posY-1),(*pacman).posX,(*pacman).posY) < dist_menor))
  	{
      GHx_menor = (*ghost).posX ;
